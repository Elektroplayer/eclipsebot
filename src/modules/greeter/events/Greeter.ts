...
export function formatObject(obj: any, f: (str: string) => string): any {
    ...
    class WelcomeListener extends Event {
        ...
        async exec(member: GuildMember) {
            ...
            if (
                ...
                !channel.permissionsFor(memberBot).has([PermissionsBitField.Flags.SendMessages, PermissionsBitField.Flags.ViewChannel])
            )
                return;

            channel.send(
                formatObject(settings.message, (str) =>
                    format(str, {
