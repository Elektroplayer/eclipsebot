...
function stringifyArray(array, prefix, postfix) {    
    var arrayClone = array.slice(); // Создаём клон массива, чтобы случайно не уничтожить основной

    if(arrayClone.length == 1) return `${prefix.replace('I', 1)}${arrayClone[0]}`;

    arrayClone = arrayClone.map((v, i) => `${prefix.replace('I', i+1)}${v}${postfix.replace('I', i+1)}`);
...
function stringifyPermissions (perms, newLines = false) {
    return stringifyArray(perms.map(v => permissions[v]), "", newLines ? "\n" : ", " );
}
...
/**
 * Форматирует строку с необходимыми значениями
 * @param {String} string 
 * @param {Object} args
 * @param {String} start
 * @param {String} end
 ...
 * // Hi, Oleg!
 */
function format(string, args, start = "{{", end = "}}") {
