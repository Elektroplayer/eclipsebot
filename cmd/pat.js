...
module.exports = {
    run: async (bot,message,args)=> {
        ...
        let response = await UTILS.getJsonResponse('https://nekos.life/api/v2/img/pat');
        ...
        if(members.length == 1) return message.channel.send(new discord.MessageEmbed().setColor(CONFIG.colors.default).setTitle(`${message.author.username} гладит ${members[0].user.username}`).setImage( response.url ).setFooter(footer));
        ...
        message.channel.send(embed.setDescription(descText)).then(msg => {
            ...
            collector.on('collect', async (m) => {
                ...
                return message.channel.send(new discord.MessageEmbed().setColor(CONFIG.colors.default).setTitle(`${message.author.username} гладит ${finMember.user.username}`).setImage(response.url).setFooter(footer));
            });
