...
module.exports = {
    run: async (bot,message,args)=> {
        ...
        if(!args[0]) return ERRORS.notArgs(message,"Упомяни того, кого хочешь погладить");

        let members = UTILS.findMembers(message, args[0]);

        if(members.length == 0) return ERRORS.noUser(message);
        if(members.length == 1) return message.channel.send(new discord.MessageEmbed().setColor(CONFIG.colors.default).setTitle(`${message.author.username} гладит ${members[0].user.username}`).setImage( (await UTILS.getJsonResponse('https://nekos.life/api/v2/img/pat')).url ).setFooter(footer));
        ...
        message.channel.send(embed.setDescription(descText)).then(msg => {
            ...
            collector.on('collect', async (m) => {
                ...
                return message.channel.send(new discord.MessageEmbed().setColor(CONFIG.colors.default).setTitle(`${message.author.username} гладит ${finMember.user.username}`).setImage((await UTILS.getJsonResponse('https://nekos.life/api/v2/img/pat')).url).setFooter(footer));
            });
    ...
    help: {
        category: "Картинки",
        arguments: "**<@user>** - Кого вы хотите погладить",
        examples: `**${CONFIG.prefix}pat @Electroplayer** - Давайте погладим Electroplayer :з`
    }
}