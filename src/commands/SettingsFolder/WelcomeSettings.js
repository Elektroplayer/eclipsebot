...
export default class WelcomeSettings extends DefaultClassSettings {
    async Init () {
        this.set = await Messages.findOne({guildID: this.guildID, type: 'Welcome'}).exec()
    }
    ...
    enabled () {
        ...
        if(!this.set) {
            this.autosetVal('welcome.message.content', '**{{USERNAME}}**, добро пожаловать на сервер {{GUILDNAME}}!')
    ...
    channelid () {
        ...
        if(!this.set) {
            this.autosetVal('welcome.enabled', 'true')
            this.autosetVal('welcome.message.content', '**{{USERNAME}}**, добро пожаловать на сервер {{GUILDNAME}}!')
